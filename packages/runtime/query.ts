...
export function inferArticleQueryIntent(request: ArticleQueryRequest): ArticleQueryIntent {
  if (request.intent) {
    ...
    if (request.intent === "keyword-search" && normalized(request.account)) {
      throw new Error("keyword-search intent cannot be combined with --account; use account-window");
    }
...
export function parseDateBoundary(value: string | undefined, edge: "after" | "before"): string | undefined {
  ...
  if (!dateOnly && !dateTime) throw new Error(`Invalid --${edge} date: ${value}`);
  const calendar = input.match(/^(\d{4})-(\d{2})-(\d{2})(?:$|T)/u);
  if (calendar) {
    const year = Number(calendar[1]);
    const month = Number(calendar[2]);
    const day = Number(calendar[3]);
    const daysInMonth = month >= 1 && month <= 12 ? new Date(Date.UTC(year, month, 0)).getUTCDate() : 0;
    if (day < 1 || day > daysInMonth) throw new Error(`Invalid --${edge} date: ${value}`);
  }
  if (dateTime) {
    const [, hour, minute, second = "0", , offsetHour = "0", offsetMinute = "0"] = dateTime;
    if (Number(hour) > 23 || Number(minute) > 59 || Number(second) > 59 ||
      Number(offsetHour) > 23 || Number(offsetMinute) > 59) {
      throw new Error(`Invalid --${edge} date: ${value}`);
    }
  }
...
function isTrustedWechatArticleUrl(value: string | undefined): boolean {
...
export function toFastArticleJson(result: ArticleSearchResult): FastArticleJson {
  const originalSource = result.sources.find((source) => isTrustedWechatArticleUrl(source.sourceUrl));
  ...
  const originalUrl = originalSource?.sourceUrl ??
    (isTrustedWechatArticleUrl(result.article.canonicalUrl) ? result.article.canonicalUrl : null);
  const discoveryUrl = originalUrl
    ? null
    : discoverySource?.sourceUrl ??
      (canonicalIsDiscovery ? safeHttpsUrl(result.article.canonicalUrl) : undefined) ??
      result.sources.map((source) => safeHttpsUrl(source.sourceUrl)).find(Boolean) ??
      safeHttpsUrl(result.article.canonicalUrl) ?? null;
  ...
  return {
    ...
    provenance: result.sources.filter((source) => safeHttpsUrl(source.sourceUrl)).map((source) => ({
      connector: source.connectorId,
      kind: source.connectorKind,
      url: safeHttpsUrl(source.sourceUrl)!,
    })),
  };
}
...
export function createFastQueryEnvelope(
  ...
  if (intent === "account-window" && request.scope !== "local") {
    warnings.push(
